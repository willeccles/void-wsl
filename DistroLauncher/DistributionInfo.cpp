...
bool DistributionInfo::CreateUser(std::wstring_view userName)
{
    ...
    DWORD exitCode;
    std::wstring commandLine = L"/usr/sbin/useradd --user-group --create-home ";
    ...
    commandLine = L"/usr/sbin/usermod -aG wheel,floppy,audio,video,cdrom,optical,kvm,xbuilder ";
    commandLine += userName;
    ...
    // Require the user to set a password.
    commandLine = L"/usr/sbin/passwd ";
    commandLine += userName;
    hr = g_wslApi.WslLaunchInteractive(commandLine.c_str(), true, &exitCode);
    if ((FAILED(hr)) || (exitCode != 0)) {
        // Delete the user if the passwd command failed.
        commandLine = L"/usr/sbin/deluser ";
        commandLine += userName;
        g_wslApi.WslLaunchInteractive(commandLine.c_str(), true, &exitCode);
        return false;
