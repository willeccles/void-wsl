...
HRESULT InstallDistribution(bool createUser)
{
    ...
    // Allow sudo usage by members of %wheel
    hr = g_wslApi.WslLaunchInteractive(L"/bin/echo '%wheel\tALL=(ALL:ALL) ALL' >> /etc/sudoers", true, &exitCode);
    if (FAILED(hr)) {
        return hr;
    }

    // Configure locales.
    hr = g_wslApi.WslLaunchInteractive(L"/usr/sbin/xbps-reconfigure -f glibc-locales", true, &exitCode);
    if (FAILED(hr)) {
        return hr;
    }
